...
### Frontend 디자인 / Figma 지침

- Frontend UI 수정 요청에서 Figma 링크가 제공되면 해당 Figma 시안을 우선 기준으로 구현한다.
- Figma를 기준으로 구현할 때도 `Task` 중심 정보 구조와 현재 앱의 화면 흐름은 유지한다.
- Figma의 시각 요소를 반영하되, 명세에 없는 새 도메인 개념이나 필수 입력값을 임의로 추가하지 않는다.
- Figma 기반 구현 전에는 가능하면 대상 frame 또는 node-id를 확인하고, 어떤 화면을 반영하는지 명확히 한다.
- Figma 시안 반영 과정에서 생긴 주요 UI 구조 변경이나 공통 스타일 규칙은 필요 시 문서 또는 코드에 근거를 남긴다.
