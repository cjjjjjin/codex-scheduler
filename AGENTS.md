...
이 프로젝트는 OpenAI Codex CLI를 사용해, 지정된 주기에 지정된 Session으로 Prompt를 전달하는 시스템이다.
...
Task 추가 시에는 외부 입력으로 Session ID를 받지 않고, 생성 과정에서 새로운 Codex Session을 만들고 그 ID를 할당해야 한다.
...
각 Task의 CRON 스케줄에 맞춰 Codex를 이용해 해당 Session에 Prompt를 전달해야 한다.
...
- Session 생성 책임은 Task 생성 플로우에 포함한다.
- Session ID는 Task 생성 후 시스템이 관리하는 값으로 취급한다.
...
- 스케줄 실행 시 어떤 Task가 언제 어떤 Session으로 전달되었는지 추적 가능해야 한다.
...
- Task 생성 화면에서는 Session ID 입력을 요구하지 않는다.
...
- Frontend에서 Session ID를 직접 입력받는 흐름을 기본 동작으로 만들지 않는다.
