...
const TASK_SETTINGS_SKILL_NAME = "task-settings";

const TASK_SETTINGS_SKILL_CONTENT = `---
name: task-settings
...
export class SkillService {
  ensureTaskSettingsSkillInstalled(workspaceDirectory: string): string {
    const skillDir = path.join(workspaceDirectory, ".agents", "skills", TASK_SETTINGS_SKILL_NAME);
    const skillPath = path.join(skillDir, "SKILL.md");

    fs.mkdirSync(skillDir, { recursive: true });

    if (!fs.existsSync(skillPath)) {
      fs.writeFileSync(skillPath, TASK_SETTINGS_SKILL_CONTENT);
    }
